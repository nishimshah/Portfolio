...
export default function Projects() {
  ...
  return (
    <section id="projects" ref={ref} className="px-4 md:px-6 max-w-7xl mx-auto py-16 bg-light-bg dark:bg-dark-bg">
      <animated.div style={fadeIn}>
        <div className="flex items-center gap-4 mb-12 justify-center md:justify-start">
          ...
          <div className="h-px bg-border-light dark:bg-border-dark flex-grow max-w-md hidden md:block"></div>
        </div>
        ...
        <div className="block md:hidden">
          <div className="flex gap-4 overflow-x-auto pb-6 snap-x snap-mandatory scrollbar-hide -mx-4 px-4">
            {projects.map((project, idx) => (
              ...
              >
                <div className="bg-gradient-to-br from-light-elevated to-accent-sand/10 dark:from-dark-card dark:to-accent-primary/5 border border-border-light dark:border-border-dark rounded-2xl overflow-hidden hover:border-accent-primary dark:hover:border-accent-sage/50 transition-all duration-300 group cursor-pointer shadow-md hover:shadow-xl h-full flex flex-col">
                  <div className="relative overflow-hidden h-48 bg-light-bg dark:bg-dark-bg flex-shrink-0">
                    <img 
                      src={project.image} 
                      alt={project.title}
                    ...
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-accent-primary/90 dark:bg-accent-sage/90 backdrop-blur-sm">
                      <div className="bg-white dark:bg-dark-card rounded-full p-4">
                        <svg className="w-8 h-8 text-accent-primary dark:text-accent-sage" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="p-4 flex flex-col flex-grow">
                    <div className="text-accent-sage dark:text-accent-sageDark text-xs mb-2 font-medium">Featured Project</div>
                    <h3 className="text-lg font-bold text-textLight-heading dark:text-text-heading mb-2 group-hover:text-accent-primary dark:group-hover:text-accent-sage transition-colors line-clamp-2">
                      {project.title}
                    </h3>
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      {project.tech.slice(0, 3).map(tech => (
                        <span key={tech} className="text-xs text-textLight-secondary dark:text-text-secondary bg-accent-sand/30 dark:bg-accent-sage/10 px-3 py-1 rounded-full">
                          {tech}
                        </span>
                      ))}
                      {project.tech.length > 3 && (
                        <span className="text-xs text-textLight-secondary dark:text-text-secondary bg-accent-sand/30 dark:bg-accent-sage/10 px-3 py-1 rounded-full">
                          +{project.tech.length - 3}
                        </span>
                      )}
                    </div>
          ...
          <p className="text-center text-sm text-textLight-secondary dark:text-text-secondary mt-4">
            ← Swipe to explore more projects →
          </p>
        </div>

        {/* Desktop Carousel */}
        <div className="hidden md:block projects-carousel max-w-5xl mx-auto">
          <Slider {...settings}>
            {projects.map((project, idx) => (
              <div key={idx} className="px-3">
                <div 
                  className="bg-gradient-to-br from-light-elevated to-accent-sand/10 dark:from-dark-card dark:to-accent-primary/5 border border-border-light dark:border-border-dark rounded-2xl overflow-hidden hover:border-accent-primary dark:hover:border-accent-sage/50 transition-all duration-300 group cursor-pointer shadow-md hover:shadow-xl flex flex-col h-[620px]"
                  onClick={() => openModal(project)}
                >
                  <div className="relative overflow-hidden h-72 bg-light-bg dark:bg-dark-bg flex-shrink-0">
                    <img 
                      src={project.image} 
                      alt={project.title}
                      className="w-full h-full object-cover"
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    <div className="text-accent-sage dark:text-accent-sageDark text-xs mb-2 font-medium">Featured Project</div>
                    <h3 className="text-2xl font-bold text-textLight-heading dark:text-text-heading mb-3 group-hover:text-accent-primary dark:group-hover:text-accent-sage transition-colors line-clamp-2">
                      {project.title}
                    </h3>
                    <p className="text-textLight-secondary dark:text-text-secondary text-sm leading-relaxed mb-4 line-clamp-3 flex-grow">
                      {project.shortDesc}
      ...
      <Modal
        ...
        className="max-w-4xl mx-auto mt-10 mb-10 p-6 md:p-8 bg-light-card dark:bg-dark-card rounded-2xl border border-border-light dark:border-border-dark outline-none max-h-[90vh] overflow-y-auto shadow-2xl"
        overlayClassName="fixed inset-0 bg-black/70 dark:bg-black/85 backdrop-blur-sm z-50 flex items-center justify-center px-4"
      >
        {selectedProject && (
          <div>
            <div className="flex justify-between items-start mb-6">
              <div>
                <div className="text-accent-sage dark:text-accent-sageDark text-sm mb-2 font-medium">Featured Project</div>
                <h2 className="text-2xl md:text-3xl font-bold text-textLight-heading dark:text-text-heading">{selectedProject.title}</h2>
              </div>
              <button
                onClick={closeModal}
                className="text-textLight-secondary dark:text-text-secondary hover:text-accent-primary dark:hover:text-accent-primaryDark transition-colors p-2 hover:bg-light-bg dark:hover:bg-dark-bg rounded-lg"
              >
            ...
            {selectedProject.video && selectedProject.video !== '#' && (
              <div className="mb-6">
                <h3 className="text-textLight-heading dark:text-text-heading font-bold mb-3 flex items-center gap-2">
                  <span className="text-xl">🎥</span>
                  Project Demo
                </h3>
                {isGoogleDriveVideo(selectedProject.video) ? (
                  <div className="relative rounded-xl overflow-hidden border border-border-light dark:border-border-dark shadow-lg" style={{ paddingBottom: '56.25%', height: 0 }}>
                    <iframe 
                  ...
                  <div className="relative rounded-xl overflow-hidden border border-border-light dark:border-border-dark shadow-lg bg-black">
                    <video 
                      controls 
                      className="w-full"
            ...
            <div className="mb-6">
              <h3 className="text-textLight-heading dark:text-text-heading font-bold mb-3 text-lg flex items-center gap-2">
                <span className="text-xl">✨</span>
                Key Features
              </h3>
              <ul className="space-y-2">
                {selectedProject.features.map((feature, idx) => (
                  <li key={idx} className="flex items-start text-textLight-secondary dark:text-text-secondary text-sm">
                    <span className="text-accent-sage dark:text-accent-sageDark mr-3 mt-1">▹</span>
                    <span>{feature}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {selectedProject.tech.map(tech => (
                  <span key={tech} className="text-sm text-textLight-secondary dark:text-text-secondary bg-accent-sand/30 dark:bg-accent-sage/10 px-4 py-2 rounded-lg border border-border-light dark:border-border-dark">
                    {tech}
                  </span>
                ))}
              </div>
            ...
            <div className="flex flex-wrap gap-4 pt-6 border-t border-border-light dark:border-border-dark">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 px-6 py-3 bg-accent-primary dark:bg-accent-sage text-white rounded-lg hover:shadow-lg hover:shadow-accent-primary/30 dark:hover:shadow-accent-sage/30 transition-all font-medium"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
                </svg>
...
function NextArrow({ onClick }) {
  return (
    <button
      onClick={onClick}
      className="hidden md:block absolute right-4 top-1/2 -translate-y-1/2 z-10 bg-accent-primary dark:bg-accent-sage text-white p-3 rounded-full hover:shadow-lg hover:shadow-accent-primary/50 dark:hover:shadow-accent-sage/50 transition-all"
...
function PrevArrow({ onClick }) {
  return (
    <button
      onClick={onClick}
      className="hidden md:block absolute left-4 top-1/2 -translate-y-1/2 z-10 bg-accent-primary dark:bg-accent-sage text-white p-3 rounded-full hover:shadow-lg hover:shadow-accent-primary/50 dark:hover:shadow-accent-sage/50 transition-all"
